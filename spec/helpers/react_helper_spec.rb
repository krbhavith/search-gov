...
describe ReactHelper do
  describe '#search_results_layout' do
    ...
    describe '#govbox_set' do
      ...
      before do
        allow(affiliate).to receive_messages(is_federal_register_document_govbox_enabled?: true, agency: agency)
      ...
      it 'filters out the created date from federal_register_documents results' do
        search_val = helper.send(:govbox_set, search)
        expect(search_val.as_json['federalRegisterDocuments'].first).not_to include('publication_date')
      end
    end
  end
end
